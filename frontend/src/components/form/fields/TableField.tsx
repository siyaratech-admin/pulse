...
export const TableField: React.FC<FormFieldProps> = ({
    ...
    const visibleFields = childFields.filter(
        f =>
            !f.hidden &&
            !['SectionBreak', 'ColumnBreak', 'TabBreak', 'HTML'].includes(f.fieldtype) &&
            (f as any).in_list_view === 1
    );
                        ...
                        return (
                            <SmallTextField
                                ...
                                className={cn(
                                    ...
                                    "[&>textarea]:min-h-[36px] [&>textarea]:h-auto [&>textarea]:py-2"
                                )}
                            />
                        );
                    }
                    ...
                    return (
                        <div className="w-full">
                            <input
                                ...
                                className="w-full px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 h-9"
                            />
                        </div>
                    );
    ...
    const calculateEndDate = useCallback(
        async (start_date: string, duration: number, project: string) => {
            ...
            try {
                const response = await fetch(
                    '/api/method/kb_planning.kb_planning.utils.get_end_date_skipping_holidays',
                    {
                        method: 'POST',
                        headers: {
                        ...
                        <div className="flex items-center justify-end">
                            {(parentDoctype === 'KB Client Baseline') ? (
                                <Button
                                    ...
                                    className="h-9 px-4 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-medium shadow-sm"
                                    type="button"
                                >
                                    ...
                                    {tableData.length === 0 ? (
                                                    ...
                                                    <div className="flex flex-col items-center gap-3">
                                                        ...
                                                        <div className="space-y-1">
                                                            ...
                                                            <p className="text-xs text-gray-500">
                                                                Click "Add Row" button above to get started
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        tableData.map((row, rowIndex) => (
                                            <TableRow
                                                ...
                                                className="hover:bg-orange-50/50 transition-colors border-b border-gray-200"
                                            >
                            ...
                            {tableData.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                    {tableData.map((row, rowIndex) => (
                                        ...
                                        >
                                            {/* Card Header */}
                                            <div className="flex items-center justify-between p-3 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-200">
            ...
            {showCreateTask && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
                        <h3 className="text-xl font-bold mb-4 bg-gradient-to-r from-orange-500 to-amber-500 bg-clip-text text-transparent">
                            Create New Baseline Task
                        </h3>

                        {!formData?.project && (
                            <div className="mb-4 p-3 bg-orange-50 border-l-4 border-orange-400 rounded-md text-sm text-orange-800">
                                <div className="flex items-start gap-2">
                                    <span className="text-lg">⚠️</span>
                                    <p>
                                        Please select a <span className="font-semibold">Project</span> in the form above to enable automatic end date calculation.
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                                    End Date
                                    {isCalculatingDate && (
                                        <span className="text-xs text-orange-500 font-normal ml-2 animate-pulse">
                                            Calculating...
                                        </span>
                                    )}
                                </label>
                                ...
                                <p className="text-xs text-gray-500 mt-1.5">
                                    Automatically calculated based on working days & holidays
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                            ...
                            <Button
                                ...
                                disabled={creatingTask}
                                className="px-5 h-10 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-medium shadow-md"
                            >
