...
const BulkTaskCreate: React.FC = () => {
  ...
  const { call: bulkCreateTasks, loading: isCreating } = useFrappePostCall<BulkCreateResult>(
    "kb_task.api.bulk_task_management.bulk_create_tasks"
  )

  // Validation constants
  const validPriorities = ["Low", "Medium", "High", "Urgent"]
  const validStatuses = ["Open", "Working", "Pending Review", "Overdue", "Completed", "Cancelled", "Template"]

  const downloadTemplate = () => {
    const templateData = [
      {
        "Subject*": "Example Task 1",
        Description: "This is an example description",
        Priority: "Medium",
        Status: "Open",
        "Expected Start Date": "2024-01-15",
        "Expected End Date": "2024-01-30",
        Progress: 0,
        "Base Points": 5,
        "Is Group": 0,
      },
    ...
    XLSX.writeFile(wb, `bulk_tasks_template_${Date.now()}.xlsx`)
  }

  const validateTask = (task: any, index: number): TaskRow => {
    const errors: string[] = []
    ...
    // Date validation
    if (task["Expected Start Date"]) {
      const date = new Date(task["Expected Start Date"])
      if (isNaN(date.getTime())) {
        errors.push("Invalid Expected Start Date format")
      }
    }

    if (task["Expected End Date"]) {
      const date = new Date(task["Expected End Date"])
      if (isNaN(date.getTime())) {
        errors.push("Invalid Expected End Date format")
      }
    }

    // Date range validation
    if (task["Expected Start Date"] && task["Expected End Date"]) {
      const startDate = new Date(task["Expected Start Date"])
      const endDate = new Date(task["Expected End Date"])
      if (startDate > endDate) {
        errors.push("Expected Start Date must be before Expected End Date")
      }
    }
    ...
    return {
      ...
      exp_start_date: task["Expected Start Date"] ? formatDate(task["Expected Start Date"]) : undefined,
      exp_end_date: task["Expected End Date"] ? formatDate(task["Expected End Date"]) : undefined,
  ...
  const handleCreateTasks = async () => {
    ...
    if (validTasks.length === 0) {
      setUploadError("No valid tasks to create")
      return
    }
      ...
      if (normalizedResult.success && normalizedResult.failed_count === 0) {
        setTimeout(() => {
          if (returnUrl) {
            ...
            navigate(`/task-manager?project=${project}`)
          }
        }, 1500)
      }
    } catch (error) {
      setUploadError(error instanceof Error ? error.message : "Failed to create tasks")
    ...
    <div className="container mx-auto py-6 px-4 max-w-7xl">
      ...
      <div className="space-y-4 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">Bulk Task Creation</h1>
              <p className="text-sm text-muted-foreground mt-1">
                Upload an Excel file to create multiple tasks
                {parentTask ? (
                  <span className="font-medium"> under parent task {parentTask}</span>
                ) : !parentTaskFromUrl ? (
                  <span> (select parent task below or leave empty for root-level tasks)</span>
                ) : (
                  <span> as root-level tasks</span>
                )}
              </p>
            </div>
          </div>
        ...
        {/* Parent Task Selector - Only show if not in URL and project is selected */}
          ...
          <div className="border-t pt-4">
            <LinkField
              field={{
                ...
                fieldtype: "Link",
                options: "Task",
                description: "Select a parent task to create sub-tasks, or leave empty for root-level tasks",
                filters: JSON.stringify([["project", "=", project], ["is_template", "=", 0]]),
              }}
        ...
        <Alert variant={createResult.success ? "default" : "destructive"} className="mb-6">
          ...
          <AlertDescription>
            ...
            {createResult.parent_converted && (
              <p className="mt-1 text-sm">Parent task has been converted to a group task.</p>
            )}
          </AlertDescription>
        </Alert>
      )}
      ...
      {!project && (
        <Alert className="mb-6">
          ...
          <AlertDescription>
            Please select a project from the dropdown above to begin bulk task creation.
          </AlertDescription>
        </Alert>
      )}
        ...
        <div className="grid md:grid-cols-2 gap-6">
            ...
            <CardContent>
              ...
              <div className="mt-4 text-sm text-muted-foreground space-y-2">
                ...
                <ul className="list-disc list-inside space-y-1 ml-2">
                  ...
                  <li>Status (Open/Working/Pending Review/etc.)</li>
                  <li>Expected Start Date (YYYY-MM-DD)</li>
                  <li>Expected End Date (YYYY-MM-DD)</li>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Upload the filled Excel file with task data</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>Preview Tasks</CardTitle>
                  <CardDescription>Review and verify tasks before creation</CardDescription>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                      ...
                      {tasks.map((task, index) => (
                        <TableRow key={index} className={!task.is_valid ? "bg-red-50" : ""}>
                          ...
                          <TableCell className="text-sm">
                            {task.exp_start_date ? new Date(task.exp_start_date).toLocaleDateString() : "-"}
                          </TableCell>
                          <TableCell className="text-sm">
                            {task.exp_end_date ? new Date(task.exp_end_date).toLocaleDateString() : "-"}
                          </TableCell>
            ...
            >
              {isCreating ? (
                ...
                <>
                  ...
                  Create {validTaskCount} Task{validTaskCount !== 1 ? "s" : ""}
                </>
              )}
            </Button>
          </div>
