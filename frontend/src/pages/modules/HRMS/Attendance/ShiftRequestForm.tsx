...
const ShiftRequestForm = () => {
    const navigate = useNavigate();
    const [fieldsData, setFieldsData] = useState<FieldMetadata[] | null>(null);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [formData, setFormData] = useState<FormData>({});
    const doctype_name = 'Shift Request';

    // Fetch current employee
    // Fetch current employee
    const { data: employeeList } = useFrappeGetDocList('Employee', {
        filters: [['user_id', '=', (window as any).frappe?.session?.user || 'Administrator']], // window.frappe usually available, or handle auth better
        fields: ['name']
    });
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center">
                            <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => navigate('/hrms/shift-requests')}
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Request a change in your shift schedule.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    {isCompleted && (
                        <Alert className="border-green-200 bg-green-50">
                            <AlertDescription className="text-green-700">
                                Shift Request submitted successfully! Redirecting...
                            </AlertDescription>
                        </Alert>
                    )}
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                onCancel={() => navigate('/hrms/shift-requests')}
