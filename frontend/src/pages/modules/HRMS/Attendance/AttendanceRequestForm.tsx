...
const AttendanceRequestForm = () => {
    ...
    const doctype_name = 'Attendance Request';
    ...
    useEffect(() => {
        if (employeeData) {
            setFormData(prev => ({ ...prev, employee: employeeData.name }));
        }
    }, [employeeData]);
    ...
    useEffect(() => {
        if (isCompleted && !createError) {
            const timer = setTimeout(() => {
                navigate('/hrms/attendance-requests');
            }, 1500);
    ...
    const fetchFormFields = async () => {
        ...
        try {
            ...
            // Filter out fields that shouldn't be shown or set defaults
            // For example, we might want to hide 'employee' if we set it automatically, 
            // but keeping it visible as read-only might be better.
            // Let's rely on the metadata for now.
    ...
    const handleFormSubmit = async (data: FormData) => {
        ...
        try {
            await createDoc(doctype_name, {
                ...data,
                employee: employeeData?.name, // Ensure employee is set
                docstatus: 0
            });
        } catch (err) {
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center">
                            <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => navigate('/hrms/attendance-requests')}
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Submit a request for attendance regularization or work from home.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    {isCompleted && (
                        <Alert className="border-green-200 bg-green-50">
                            <AlertDescription className="text-green-700">
                                Attendance Request submitted successfully! Redirecting...
                            </AlertDescription>
                        </Alert>
                    )}
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                onCancel={() => navigate('/hrms/attendance-requests')}
