...
const NewRccHandoverForm: React.FC = () => {
    ...
    // Fetch RCC Handover DocType fields on component mount
    useEffect(() => {
        fetchRccHandoverFields();
    }, []);
    ...
    const fetchRccHandoverFields = async () => {
        console.log('Fetching RCC Handover DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('RCC Handover fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'RCC Handover fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray);
        } catch (err) {
            console.error('Error fetching RCC Handover fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load RCC Handover fields');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFormSubmit = async (submittedFormData: FormData) => {
        console.log('Creating new RCC Handover with data:', submittedFormData);
        setError(null);
        reset(); // Reset any previous state from the hook
        
        try {
            // Use the Frappe React SDK hook to create the document
            const result = await createDoc('RCC Handover', submittedFormData);
            console.log('RCC Handover created successfully:', result);
            // Navigation is handled in useEffect when isCompleted becomes true
        } catch (err) {
            console.error('RCC Handover creation error:', err);
            // The createError from the hook will also be set automatically
            setError(err instanceof Error ? err.message : 'Failed to create RCC Handover record');
        }
    };
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <h1 className="text-xl font-bold text-gray-900 tracking-tight">
                                    New RCC Handover Record
                                </h1>
                            </div>
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Create a comprehensive RCC (Reinforced Cement Concrete) handover record to document construction completion, quality inspections, and formal handover procedures. Track project details, inspection checklists, cube strength tests, and approval workflows.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading RCC Handover Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching DocType fields and preparing the handover documentation form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                                    ...
                                    <div className="flex gap-2 ml-4 shrink-0">
                                        {error && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={fetchRccHandoverFields}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">RCC Handover Record Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                title="RCC Handover Details"
                                description="Document comprehensive RCC handover process including template selection, project information, inspection details, strength testing results, corrective actions, and formal approval signatures for construction completion certification."
                            />
                        </div>
                    )}
                </div>
...
export default NewRccHandoverForm;