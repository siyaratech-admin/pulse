...
const NewDimensionsForm: React.FC = () => {
    ...
    // Fetch Dimensions DocType fields on component mount
    useEffect(() => {
        fetchDimensionsFields();
    }, []);
    ...
    const fetchDimensionsFields = async () => {
        console.log('Fetching Dimensions DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('Dimensions fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'Dimensions fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray);
        } catch (err) {
            console.error('Error fetching Dimensions fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load Dimensions fields');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFormSubmit = async (submittedFormData: FormData) => {
        console.log('Creating new Dimensions with data:', submittedFormData);
        setError(null);
        reset(); // Reset any previous state from the hook
        
        try {
            // Use the Frappe React SDK hook to create the document
            const result = await createDoc('Dimensions', submittedFormData);
            console.log('Dimensions created successfully:', result);
            // Navigation is handled in useEffect when isCompleted becomes true
        } catch (err) {
            console.error('Dimensions creation error:', err);
            // The createError from the hook will also be set automatically
            setError(err instanceof Error ? err.message : 'Failed to create Dimensions record');
        }
    };
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <h1 className="text-xl font-bold text-gray-900 tracking-tight">
                                    New Dimensions Record
                                </h1>
                            </div>
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Create a comprehensive dimensions record to document structural measurements, site details, and quality control signatures. Track dimension types, location details, and approval workflows for construction projects.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading Dimensions Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching DocType fields and preparing the measurement form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">Dimensions Record Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                description="Document structural dimensions and measurements with comprehensive location details. Include dimension type selection, site information, measurement tables, and multi-level approval signatures."
                            />
                        </div>
                    )}
                </div>
...
export default NewDimensionsForm;