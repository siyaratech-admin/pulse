...
const NewMaterialInspectionForm: React.FC = () => {
    const navigate = useNavigate();
    const { id: urlId } = useParams<{ id?: string }>();

    const doctype_name = 'Material Inspection';

    // 1. Logic to handle Edit vs Create mode
    ...
    const {
        ...
        error: docFetchError
    } = useFrappeGetDoc(doctype_name, documentId, {
        enabled: !!documentId
    });
    ...
    // --- Logic: Metadata Processing (Hidden fields and depends_on) ---
        ...
        const fetchMeta = async () => {
            ...
            try {
                const response = await fetch('/api/v2/method/pulse.api.get_fields_of_doctype.get_form_meta', {
                    ...
                    body: JSON.stringify({ doctype_name: doctype_name })
                });
                ...
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                            ...
                            {isEditMode ? `Edit Material Inspection` : 'New Material Inspection'}
                        </h1>
                    </div>
            ...
            <div className="max-w-5xl mx-auto p-6">
                ...
                {isSuccess && (
                    <Alert className="mb-6 bg-emerald-50 border-emerald-200 text-emerald-800">
                        <AlertDescription className="font-medium">
                            Material inspection saved successfully. Redirecting...
                        </AlertDescription>
                    </Alert>
                )}
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-32 bg-white border rounded-xl shadow-sm">
                        ...
                        <p className="text-gray-500 animate-pulse">Synchronizing with server...</p>
                    </div>
                ) : (
                    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                            <div>
                                <h2 className="text-sm font-bold text-gray-700 uppercase tracking-tight">Inspection Details</h2>
                                <p className="text-xs text-gray-500 mt-1">Please ensure all material quality checks are documented.</p>
                            </div>
                        ...
                        <div className="px-6 py-3 bg-gray-50 border-t border-gray-100 flex justify-between">
                            <p className="text-[10px] text-gray-400 uppercase tracking-widest">
                                System Ready | Doctype: {doctype_name}
                            </p>
...
export default NewMaterialInspectionForm;