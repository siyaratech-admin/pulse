...
const NewStockEntryForm: React.FC = () => {
    ...
    // Fetch Stock Entry DocType fields on component mount
    useEffect(() => {
        fetchStockEntryFields();
    }, []);
    ...
    const fetchStockEntryFields = async () => {
        console.log('Fetching Stock Entry DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('Stock Entry fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'Stock Entry fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray.filter((f: any) => !f.hidden));
        } catch (err) {
            console.error('Error fetching Stock Entry fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load Stock Entry fields');
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Create a comprehensive stock entry to manage inventory movements for plant and machinery materials.
                                Record material receipts, issues, transfers between warehouses, manufacturing entries, and material consumption.
                                Track stock quantities, manage BOM details, process loss, warehouse transfers, and maintain complete audit trails for all inventory transactions.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading Stock Entry Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching Stock Entry DocType fields and preparing the form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">Stock Entry Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                description="Complete all required fields for the stock transaction. Include stock entry type, purpose, related documents (Work Order, Purchase Order, Delivery Note, etc.), BOM information, source and target warehouse details, item quantities, process loss information, posting date/time, inspection requirements, and additional reference information for comprehensive inventory tracking."
                            />
                        </div>
                    )}
                </div>
...
export default NewStockEntryForm;
