...
const NewEhsAndToolBoxForm: React.FC = () => {
    ...
    // Fetch EHS and Tool Box DocType fields on component mount
    useEffect(() => {
        fetchEhsAndToolBoxFields();
    }, []);
    ...
    const fetchEhsAndToolBoxFields = async () => {
        console.log('Fetching EHS and Tool Box DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('EHS and Tool Box fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'EHS and Tool Box fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray);
        } catch (err) {
            console.error('Error fetching EHS and Tool Box fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load EHS and Tool Box fields');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFormSubmit = async (data: FormData) => {
        console.log('Creating new EHS and Tool Box record with data:', data);
        setError(null);
        reset();

        try {
            // Use the Frappe React SDK hook to create the document
            const result = await createDoc('EHS and Tool Box', { ...data });
            console.log('EHS and Tool Box record created successfully:', result);
            // Navigation is handled in useEffect when isCompleted becomes true
        } catch (err) {
            console.error('EHS and Tool Box creation error:', err);
            // The createError from the hook will also be set automatically
            setError(err instanceof Error ? err.message : 'Failed to create EHS and Tool Box record');
        }
    };
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <h1 className="text-xl font-bold text-gray-900 tracking-tight">
                                    Create New EHS and Tool Box Record
                                </h1>
                            </div>
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Create a new EHS (Environmental Health and Safety) and Tool Box record. Complete all required fields marked with an asterisk (*) to document safety training and tool box talks.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading EHS and Tool Box Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching DocType fields and preparing the form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                                    ...
                                    <div className="flex gap-2 ml-4 shrink-0">
                                        {error && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={fetchEhsAndToolBoxFields}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">EHS and Tool Box Record Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                title="EHS and Tool Box Details"
                                description="Complete the EHS and Tool Box training record. Include all participants, topics discussed, and required documentation."
...
export default NewEhsAndToolBoxForm;