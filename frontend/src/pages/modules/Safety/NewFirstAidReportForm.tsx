...
const NewFirstAidReportForm: React.FC = () => {
    ...
    // Fetch First Aid Report DocType fields on component mount
    useEffect(() => {
        fetchFirstAidReportFields();
    }, []);

    // Handle successful first aid report creation
    useEffect(() => {
    ...
    const fetchFirstAidReportFields = async () => {
        console.log('Fetching First Aid Report DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('First Aid Report fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'First Aid Report fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray);
        } catch (err) {
            console.error('Error fetching First Aid Report fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load First Aid Report fields');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFormSubmit = async (data: FormData) => {
        console.log('Creating new First Aid Report with data:', data);
        setError(null);
        reset();
        
        try {
            // Use the Frappe React SDK hook to create the document
            const result = await createDoc('First Aid Report', { ...data });
            console.log('First Aid Report created successfully:', result);
            // Navigation is handled in useEffect when isCompleted becomes true
        } catch (err) {
            console.error('First Aid Report creation error:', err);
            // The createError from the hook will also be set automatically
            setError(err instanceof Error ? err.message : 'Failed to create first aid report');
        }
    };
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <h1 className="text-xl font-bold text-gray-900 tracking-tight">
                                    Create New First Aid Report
                                </h1>
                            </div>
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Document first aid treatment and medical assistance provided. All required fields are marked with an asterisk (*) and must be completed before submission.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading First Aid Report Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching First Aid Report DocType fields and preparing the form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                                    ...
                                    <div className="flex gap-2 ml-4 shrink-0">
                                        {error && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={fetchFirstAidReportFields}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">First Aid Report Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                title="First Aid Report Details"
                                description="Complete all required fields to document the first aid treatment. Fields marked with (*) are mandatory."
...
export default NewFirstAidReportForm;