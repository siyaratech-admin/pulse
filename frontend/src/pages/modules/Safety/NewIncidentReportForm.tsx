...
const NewIncidentReportForm: React.FC = () => {
    ...
    // Fetch Incident Report DocType fields on component mount
    useEffect(() => {
        fetchIncidentReportFields();
    }, []);

    // Handle successful incident report creation
    useEffect(() => {
    ...
    const fetchIncidentReportFields = async () => {
        console.log('Fetching Incident Report DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('Incident Report fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'Incident Report fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray);
        } catch (err) {
            console.error('Error fetching Incident Report fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load Incident Report fields');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFormSubmit = async (data: FormData) => {
        console.log('Creating new Incident Report with data:', data);
        setError(null);
        reset();
        
        try {
            // Use the Frappe React SDK hook to create the document
            const result = await createDoc('Incident Report', { ...data });
            console.log('Incident Report created successfully:', result);
            // Navigation is handled in useEffect when isCompleted becomes true
        } catch (err) {
            console.error('Incident Report creation error:', err);
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Document safety incidents and near-miss events. All required fields are marked with an asterisk (*) and must be completed before submission.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading Incident Report Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching Incident Report DocType fields and preparing the form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                                    ...
                                    <div className="flex gap-2 ml-4 shrink-0">
                                        {error && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={fetchIncidentReportFields}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">Incident Report Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                description="Complete all required fields to document the incident. Fields marked with (*) are mandatory."
...
export default NewIncidentReportForm;