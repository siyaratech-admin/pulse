...
const NewPreMedicalForm: React.FC = () => {
    ...
    // Fetch Pre Medical Form DocType fields on component mount
    useEffect(() => {
        fetchPreMedicalFormFields();
    }, []);

    // Handle successful pre medical form creation
    useEffect(() => {
    ...
    const fetchPreMedicalFormFields = async () => {
        console.log('Fetching Pre Medical Form DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('Pre Medical Form fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'Pre Medical Form fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray.filter((f: any) => !f.hidden));
        } catch (err) {
            console.error('Error fetching Pre Medical Form fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load Pre Medical Form fields');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFormSubmit = async (data: FormData) => {
        console.log('Creating new Pre Medical Form with data:', data);
        setError(null);
        reset();

        try {
            // Use the Frappe React SDK hook to create the document
            const result = await createDoc('Pre Medical Form', { ...data });
            console.log('Pre Medical Form created successfully:', result);
            // Navigation is handled in useEffect when isCompleted becomes true
        } catch (err) {
            console.error('Pre Medical Form creation error:', err);
            // The createError from the hook will also be set automatically
            setError(err instanceof Error ? err.message : 'Failed to create pre medical form');
        }
    };
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <h1 className="text-xl font-bold text-gray-900 tracking-tight">
                                    Create New Pre Medical Form
                                </h1>
                            </div>
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Complete pre-employment medical fitness certification and health assessment. All required fields are marked with an asterisk (*) and must be completed before submission.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading Pre Medical Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching Pre Medical Form DocType fields and preparing the form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                                    ...
                                    <div className="flex gap-2 ml-4 shrink-0">
                                        {error && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={fetchPreMedicalFormFields}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">Pre Medical Form Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                title="Pre Medical Form Details"
                                description="Complete all required fields for medical fitness certification. Fields marked with (*) are mandatory."
...
export default NewPreMedicalForm;