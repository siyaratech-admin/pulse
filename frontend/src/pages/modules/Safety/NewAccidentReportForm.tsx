...
const NewAccidentReportForm: React.FC = () => {
    ...
    // Fetch Accident Report DocType fields on component mount
    useEffect(() => {
        fetchAccidentReportFields();
    }, []);

    // Handle successful accident report creation
    useEffect(() => {
    ...
    const fetchAccidentReportFields = async () => {
        console.log('Fetching Accident Report DocType fields...');
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            console.log('Accident Report fields API Response:', data);
            ...
            console.log('Loaded', fieldsArray.length, 'Accident Report fields');
            console.log('Sample field data:', fieldsArray[0]); // Debug field structure
            setFieldsData(fieldsArray);
        } catch (err) {
            console.error('Error fetching Accident Report fields:', err);
            setError(err instanceof Error ? err.message : 'Failed to load Accident Report fields');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleFormSubmit = async (data: FormData) => {
        console.log('Creating new Accident Report with data:', data);
        setError(null);
        reset();
        
        try {
            // Use the Frappe React SDK hook to create the document
            const result = await createDoc('Accident Report', { ...data });
            console.log('Accident Report created successfully:', result);
            // Navigation is handled in useEffect when isCompleted becomes true
        } catch (err) {
            console.error('Accident Report creation error:', err);
            // The createError from the hook will also be set automatically
            setError(err instanceof Error ? err.message : 'Failed to create accident report');
        }
    };
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-gray-600 text-base max-w-3xl leading-relaxed">
                                Document accident details and safety incidents. All required fields are marked with an asterisk (*) and must be completed before submission.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex items-center justify-center py-20">
                            <div className="text-center space-y-4">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-lg font-semibold text-gray-900">Loading Accident Report Form</h3>
                                    <p className="text-gray-600 text-sm">Fetching Accident Report DocType fields and preparing the form...</p>
                                </div>
                            </div>
                        </div>
                    )}
                                    ...
                                    <div className="flex gap-2 ml-4 shrink-0">
                                        {error && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={fetchAccidentReportFields}
                        ...
                        <Alert className="border-green-200 bg-green-50 rounded-sm">
                            <AlertDescription className="text-green-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-semibold text-green-800 mb-1">Accident Report Created Successfully!</div>
                    ...
                    {fieldsData && !isLoading && (
                        <div className="bg-white rounded-sm border border-gray-200 overflow-hidden">
                            <DynamicForm
                                ...
                                description="Complete all required fields to document the accident. Fields marked with (*) are mandatory."
...
export default NewAccidentReportForm;